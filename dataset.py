...
class HS_multiscale_DSet(Dataset):
    def __init__(self, opt):                                   		    # root: list ; transform: torch transform
        ...
        self.baseroot_A = os.path.join(opt.baseroot, 'HSI')
        self.baseroot_B = os.path.join(opt.baseroot, 'RGB')
        if opt.save_path == 'track1':
            self.baseroot_B = os.path.join(opt.baseroot, 'RGB')
            #print(os.path.join(opt.baseroot, 'RGB'))
            #print('s')
    ...
    def get_names(self, path):
        ...
        for root, dirs, files in os.walk(path):  
            for filespath in files: 
                #ret.append(filespath[:10])          # e.g. ARAD_HS_0001
                ret.append(Path(filespath).stem)          # e.g. ARAD_HS_0001
        return ret
